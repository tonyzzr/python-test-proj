...
try:
    # modern skimage exposes metrics functions under skimage.metrics
...
def write_psnr(pred_img, gt_img, writer, iter, prefix):
    ...
    for i in range(batch_size):
        ...
        # Use skimage.metrics when available (newer API). Fall back to skimage.measure compat functions.
        if skimage_metrics is not None:
            # structural_similarity uses channel_axis instead of multichannel in newer versions
            try:
                ssim = skimage_metrics.structural_similarity(p, trgt, channel_axis=2, data_range=1)
            except TypeError:
                # older skimage may accept multichannel kwarg
                ssim = skimage_metrics.structural_similarity(p, trgt, multichannel=True, data_range=1)
            psnr = skimage_metrics.peak_signal_noise_ratio(p, trgt, data_range=1)
        else:
